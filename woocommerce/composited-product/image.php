...
?><div class="composited_product_images"><?php
	...
	if ( has_post_thumbnail( $product_id ) ) {
		...
		$image_data    = wp_get_attachment_image_src( $image_post_id, 'catalog' );
		$image_link    = $image_data[ 0 ];
		$image         = get_the_post_thumbnail( $product_id, apply_filters( 'woocommerce_composited_product_thumbnail_size', 'full' ), array(
			'title'                   => $image_title,
		...
		$html .= sprintf( '<a href="%1$s" class="placeholder_image zoom" data-rel="%3$s"><img src="%1$s" alt="%2$s"/></a>', wc_placeholder_img_src(), __( 'Product placeholder image', 'woocommerce-composite-products' ), $image_rel );
		$html .= '</figure>';
	}
